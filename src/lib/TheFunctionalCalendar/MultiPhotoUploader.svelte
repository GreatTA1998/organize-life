<script>
      ...
      if (imageBlobFile) {
        ...
        promises.push(
          uploadImageBlobToFirebase(imageBlobFile, id).then(resultSnapshot => {
            createNewScheduledTaskContainingImage(
              resultSnapshot,
              imageBlobFile,
              id
            )
  ...
  async function createNewScheduledTaskContainingImage(
    resultSnapshot,
    imageBlobFile,
    id
  ) {
    ...
    const p2 = getDownloadURL(ref(storage, fullPath)).then(
      url => (imageDownloadURL = url)
    )

    await Promise.all([p1, p2]).catch(err =>
      console.error(
        'error in creNewSchedueledTasksContainingImage PromiseAll',
        err
      )
    )
    ...
    let dateClassObj
    if (imageBlobFile.lastModified)
      dateClassObj = new Date(imageBlobFile.lastModified)
    ...
    await setFirestoreDoc(`users/${$user.uid}/tasks/${id}`, formattedTask)
...
<div style="display: flex; align-items: center;">
  ...
  <span
    on:click={openFolderInput}
    class="material-symbols-outlined responsive-icon-size"
  >
    photo_library
  </span>
